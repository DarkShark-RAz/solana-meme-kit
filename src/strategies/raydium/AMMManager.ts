  ...
  async initialize(
    ...
    try {
      ...
      const baseAmount = new BN(
        tokenLiquidityAmount * Math.pow(10, options.decimals || 6)
      );
      const quoteAmount = new BN(solLiquidityAmount * Math.pow(10, 9));

      Logger.info(
        `Creating AMM Pool for Market: ${marketId.toBase58().slice(0, 8)}...`
      );
      ...
      const poolId = new PublicKey(
        (extInfo as any).address.ammId || (extInfo as any).address.poolId
      );
      ...
      Logger.info(
        `AMM Pool Creation: ${instructions.length} instructions generated.`
      );
